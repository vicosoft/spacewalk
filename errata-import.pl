...
#   &debug("--include-channels set: ".join(" ", @includechannels)."\n");
...
#   &debug("--exclude-channels set: ".join(" ", @excludechannels)."\n");
...
foreach $advisory (sort(keys(%{$xml}))) {
  ...
  if (not(defined($existing{$advid}))) {
    ...
    if ( ref($xml->{$advisory}->{packages}) eq 'ARRAY') {
      foreach $package ( @{$xml->{$advisory}->{packages}} ) {
